...
Compra 5 negada. Saldo insuficiente e limite de crédito excedido. //RESPOSTA CORRETA
...
```
INICIO
    var int idade <- prompt("Digite sua idade: ")
    SE (IDADE < 16) ENTÃO
        RETORNA "Não pode votar"
    FIM DO SE
    SE NÃO SE (IDADE >= 16 E IDADE < 18) ENTÃO
        RETORNA "Voto facultativo"
    FIM DO SE NÃO SE
    SE NÃO ENTÃO
        RETORNA "Voto obrigatório"
    FIM DO SE NÃO
FIM
```

````JS
const verificarSituacaoDeVoto = () => {
...
Classe Circulo Herda FormaGeometrica:
    ...
    Método construtor(cor, raio):
        super(cor) //PARA PEGAR O CONSTRUTOR DA CLASSE PAI
        Define o valor do atributo cor com o valor passado como parâmetro.
...
```
INICIO
    var FLUTUANTE distanciaCorridaEmKm <- 4,309
    ...
    ENQUANTO (tempoCalculado >= metaTempoEmSegundos) FACA
        ...
        tempoEmAceleracaoAteVelocidadeMaxima <- velocidadeMaximaEmMetrosPorSegundo / aceleracao
...
````
Funcao multiplicarMatrizes(matrizA, matrizB)
    SE tamanhoColunas(matrizA) != tamanhoLinhas(matrizB) FACA
        RETORNA "Não é possível multiplicar as duas matrizes"
    FIM DO SE
    colunaB <- 0
    Para i de 0 até matrizA.tamanho - 1
        Para j de 0 até matrizB[0].tamanho - 1
            soma <- 0
            Para k de 0 até matrizA[0].tamanho - 1
